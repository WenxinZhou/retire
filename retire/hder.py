...
class cv_reg_er(reg_er):
    ...
    def fit(self, lambda_seq=np.array([]), nlambda=40, tau=0.5, tune=0, nfolds=5, penalty="SCAD", a=3.7, nstep=5, standardize=True, adjust=True):
        ...
        plr = reg_er(self.X, self.Y, self.itcp)

        if not lambda_seq.any():
            lambda_seq = plr.lambda_seq(nlambda, standardize=standardize)
        else:
        ...
        for v in range(nfolds):
            ...
            train = reg_er(X_train, Y_train, intercept=self.itcp)
        ...
        if penalty == "L1":
            cv_beta, cv_fit = plr.l1(lambda_min, tau, tune, standardize=standardize, adjust=adjust)
        else:
            cv_beta, cv_fit = plr.irw(lambda_min, tau, tune, penalty=penalty, a=a, nstep=nstep, standardize=standardize, adjust=adjust)
